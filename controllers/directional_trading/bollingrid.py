...
class BollinGridControllerConfig(DirectionalTradingControllerConfigBase):
    ...
    def validate_grid_geometry(self):
        # CLA-014 (Codex correction): the limit price is the risk stop beyond the
        # grid start; a limit coefficient at or inside the start coefficient makes
        # the stop trigger inside the grid itself.
        if self.grid_limit_price_coefficient <= self.grid_start_price_coefficient:
            raise ValueError(
                f"grid_limit_price_coefficient ({self.grid_limit_price_coefficient}) must be "
                f"greater than grid_start_price_coefficient ({self.grid_start_price_coefficient})")
        return self
...
class BollinGridController(DirectionalTradingControllerBase):
    ...
    async def update_processed_data(self):
        ...
        # CDX-001 / CLA-405: fail closed to signal=0 (and no grid geometry) on
        # stale/absent candles instead of pricing a grid off a frozen bar.
