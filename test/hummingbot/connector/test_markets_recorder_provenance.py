...
class TestMarketsRecorderProvenanceEnrichment(unittest.TestCase):
    ...
    def test_fill_source_channel_from_tracked_order(self):
        """Test that source_channel is read from _fill_sources on tracked order."""
        # Simulate a tracked order with _fill_sources
        mock_order = MagicMock()
        mock_order._fill_sources = {"trade_123": "ws", "trade_456": "rest_poll"}

        self.assertEqual(mock_order._fill_sources.get("trade_123", "unknown"), "ws")
        self.assertEqual(mock_order._fill_sources.get("trade_456", "unknown"), "rest_poll")
        self.assertEqual(mock_order._fill_sources.get("trade_789", "unknown"), "unknown")
