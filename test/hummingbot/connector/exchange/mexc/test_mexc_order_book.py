...
class MexcOrderBookTests(TestCase):
    ...
    def test_diff_message_from_exchange(self):
        ...
        # update_id should use toVersion (as int) for sequencing, not timestamp
        self.assertEqual(17521975455, diff_msg.update_id)
        ...
        self.assertEqual(17521975455, diff_msg.bids[0].update_id)
        ...
        self.assertEqual(17521975455, diff_msg.asks[0].update_id)
    ...
    def test_diff_message_uses_version_not_timestamp(self):
        """update_id must use toVersion, not sendTime/timestamp."""
        diff_msg = MexcOrderBook.diff_message_from_exchange(
            msg={
                "publicAggreDepths": {
                    "bids": [],
            ...
            timestamp=float("99999"),
            metadata={"trading_pair": "BTC-USDC"}
        )
        self.assertEqual(12345, diff_msg.update_id)

    def test_diff_message_includes_first_update_id(self):
        """first_update_id must be populated from fromVersion as int."""
        diff_msg = MexcOrderBook.diff_message_from_exchange(
            msg={
                "publicAggreDepths": {
                    "bids": [],
        ...
        self.assertEqual(12344, diff_msg.content.get("first_update_id"))

    def test_diff_message_falls_back_to_timestamp_when_no_version(self):
        """When no version fields, fall back to timestamp as int."""
        diff_msg = MexcOrderBook.diff_message_from_exchange(
            msg={
                "publicAggreDepths": {
                    "bids": [],
        ...
        self.assertEqual(99999, diff_msg.update_id)

    def test_snapshot_update_id_is_integer(self):
        """snapshot update_id must be int, not str."""
        snapshot_msg = MexcOrderBook.snapshot_message_from_exchange(
            msg={"lastUpdateId": "12345678", "bids": [], "asks": []},
            timestamp=1640000000.0,
            metadata={"trading_pair": "BTC-USDC"}
        )
        self.assertIsInstance(snapshot_msg.update_id, int)
        self.assertEqual(12345678, snapshot_msg.update_id)

    def test_diff_update_id_is_integer(self):
        """diff update_id and first_update_id must be int, not str."""
        diff_msg = MexcOrderBook.diff_message_from_exchange(
            msg={
                "publicAggreDepths": {
                    "bids": [], "asks": [],
        ...
        self.assertIsInstance(diff_msg.update_id, int)
        self.assertIsInstance(diff_msg.content.get("first_update_id"), int)
        self.assertEqual(17521975455, diff_msg.update_id)
        self.assertEqual(17521975448, diff_msg.content.get("first_update_id"))

    def test_diff_first_update_id_none_when_no_from_version(self):
        """first_update_id should be None when fromVersion absent."""
        diff_msg = MexcOrderBook.diff_message_from_exchange(
            msg={
                "publicAggreDepths": {
                    "bids": [], "asks": [],
        ...
        self.assertEqual(12345, diff_msg.update_id)
        self.assertIsNone(diff_msg.content.get("first_update_id"))
