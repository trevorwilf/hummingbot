...
Usage:  python nonkyc_auth_test.py
Requires NONKYC_API_KEY and NONKYC_API_SECRET in .env at the repo root.
"""
import hashlib
import hmac
import json
...
def run_get_test(api_key, api_secret, endpoint, params=None, label=""):
...
def run_post_test(api_key, api_secret, endpoint, body, label=""):
    url = f"{BASE_URL}{endpoint}"
    nonce = str(int(time.time() * 1e3))
...
def main():
    ...
    s1 = run_get_test(api_key, api_secret, "/balances", label="(no params)")

    # Test 2: GET /account/orders with params
    s2 = run_get_test(api_key, api_secret, "/account/orders", params={"status": "active"}, label="(with params)")

    # Test 3: GET /account/trades
    s3 = run_get_test(api_key, api_secret, "/account/trades", label="(trade history)")
