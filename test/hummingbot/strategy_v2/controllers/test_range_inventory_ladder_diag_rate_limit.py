...
def _make_buy_controller(buy_prices, blocked=None, best_bid="340"):
    ...
    # Refresh-wave gates: no wave in flight for these filter-event tests.
    controller._wave_cancels_in_flight = MagicMock(return_value=False)
    controller._wave_balance_gate_active = MagicMock(return_value=False)
    controller._heal_scope_levels = MagicMock(return_value=None)
