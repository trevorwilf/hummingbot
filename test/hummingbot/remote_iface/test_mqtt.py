...
class RemoteIfaceMQTTTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        # No strategy loaded by default. This fork reads strategy state from
        # TradingCore, so the fixtures set trading_core.strategy (not hbapp.strategy).
    ...
    def test_mqtt_rpc_response_envelope_is_wire_compatible(self):
        ...
        # reply_to round-trip; assert the response envelope matches the preserved
        # wire format (including the historical 'agent' field value below).
