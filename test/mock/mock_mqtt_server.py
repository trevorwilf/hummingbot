...
class FakeMQTTMessage:
    ...
    ``envelope=True`` wraps the payload in the RPC ``{header, data}`` envelope
    (for command/RPC requests); ``envelope=False`` sends the payload verbatim
    (for plain pub/sub messages such as external events).
    """
...
class FakeMQTTBroker:
    ...
    def publish_to_subscription(self, topic: str, payload: Any):
        """Inject an inbound RPC request (wrapped in the {header, data} envelope)."""
