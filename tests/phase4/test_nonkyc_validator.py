...
def _invoke(adapter, arr):
    """
    Populate `self._candles` to match the input length so the validator's
    `len(self._candles) <= 1` early-return does not skip validation.
    In live usage, the deque and the `candles` argument always have matching
    row counts (both paths at candles_base.py:185 and :344 guarantee this).
    """
    adapter._candles.clear()
    for row in arr:
        adapter._candles.append(row.tolist())
    adapter.check_candles_sorted_and_equidistant(arr)


def test_empty_and_single_row_skip_without_reset(adapter):
    _invoke(adapter, _mk([]))
    _invoke(adapter, _mk([1_700_000_000]))
