...
> **[CORRECTION 2026-07-16 — hb_predeploy adversarial review + scope triage]** — **this TL;DR is refuted on both of its load-bearing claims and must not be used as a ship gate.** Corrections 5 and 2 follow the struck text; §§1–4 of this review (structural verification) were re-verified by both engines and **hold**.
...
> **[CORRECTION 2026-07-16 — hb_predeploy adversarial review + scope triage]** (correction 5) — **"Ship-ready" is REFUTED.** Two independent engines (Codex `gpt-5.6-sol`, Claude) cross-confirmed two blockers this review does not list:
> - **CDX-001 / CDX-M04 (High):** a failed resume `shutil.rmtree`s a **pre-existing** target instance dir — the `DEST_NOT_EMPTY` guard meant to protect a non-empty destination is exactly what triggers the delete; `_cleanup_failed_instance` carries no `created_by_this_attempt` flag, so it deletes what it did not create. **Data loss.** (Claude narrowed reachability — two deploys of the same base name inside one wall-clock second, names being second-granular at `bot_orchestration.py:503-506` — and downgraded Critical→High; the defect itself is unconditional.)
> - **CDX-002 (Critical):** the stack bind-mounts a stale host `docker_service.py` over the image's, so **the rebuilt hook this review just approved does not run in the container**; the init writes the new upstream checksum *before* the retain decision, so the "upstream changed" warning self-silences after one restart. This defeats the entire subject of this review. (Fixed in the engine repo, batch phase 1.)
>
> **"None are triggerable / all but one fail closed" is REFUTED** (correction 2) — there are **two** reachable fail-opens, and #3 names the wrong trigger for one:
> 1. **empty / whitespace `id:`** (CDX-008 / CLA-002, High) — see the correction at §5.1 #3;
> 2. **absolute `state_file_name`** (CDX-007 / CLA-004, High) — see the correction at §5.1 "vectors tried and SAFE", where this review clears it.
>
> Sources: `scope_triage.md` (Batch 0, Batch 1, "Doc corrections owed"); `REPORT_hb_predeploy.md` §1.1, §1.2, §1.4, §4.1, §4.3, §4.6, §4.7.
...
> **[CORRECTION 2026-07-16 — hb_predeploy adversarial review + scope triage]** (correction 2) — **row #3's trigger is wrong, and it is not the only fail-open.**
>
> Row #3 (and its footnote) reason from **Pydantic v1** coercion semantics. **Probed directly under the installed Pydantic 2.12.5**, against the engine at review revision `47c714a19`:
>
> | staged `id:` | engine `ControllerConfigBase` | reachable fail-open? |
> |---|---|---|
...
> **[CORRECTION 2026-07-16 — hb_predeploy adversarial review + scope triage]** (correction 4, applies to row **#2** above) — row #2's `[""]` half is wrong: `""` is already rejected **422** at the model layer (`models/bot_orchestration.py:124-125`) and never reaches `_plan_extra_paths`. The reachable trigger is `"."` — **and `"./"`, which this row misses**. Row #2 is otherwise accurate. **OUT OF SCOPE / accepted risk** (CDX-014 / CLA-005) — correcting the trigger only; not queued for a fix. Full detail at the §6 correction. Sources: `scope_triage.md` OUT OF SCOPE (CDX-014 / CLA-005); `REPORT_hb_predeploy.md` §4.11.

**Copy-plan vectors tried and SAFE:** ~~absolute `state_file_name` (POSIX slash, backslash, drive-letter — correct on both platforms) → `absolute_skipped`~~ **← REFUTED, see correction 1 below**; absolute `resume_extra_paths` → `EXTRA_PATH_ESCAPE`; `../..` / symlink-out / missing extra_paths → fail-closed; empty-JSON ledger (`{}`/`[]`/`null`) → copied, judged acceptable-by-design (engine quarantine handles structural validity; rejecting `{}` would false-positive on legit fresh states); orphan-scan regex `range_inventory_ladder_(.+)\.json` → no misclassification.

> **[CORRECTION 2026-07-16 — hb_predeploy adversarial review + scope triage]** (correction 1) — **absolute `state_file_name` is NOT a safe vector. `absolute_skipped` is the second reachable fail-open** (CDX-007 / CLA-004, High; cross-confirmed unanimously, high confidence). This review clearing it as "correct on both platforms" is what made the TODO's "NOT a bug — do not fix" line, which both engines called the most consequential error in the doc pair.
>
> The skip is not a decision that the path is safe — it is the hook **proceeding on an unverifiable assumption**, contradicting its own containment invariant. Verified end-to-end:
> 1. `resume_service.py:684-693` — records `absolute_skipped` / `decisions[controller_id] = "skipped"`, warns "shared-mount scheme assumed", and `return None`. No abort.
> 2. `docker_service.py:327-341` — the `volumes` dict is a **fixed** set (`conf/`, `conf/connectors`, `conf/scripts`, `conf/controllers`, `data/`, `logs/`, shared `scripts`/`controllers`, `certs` ro). **No arbitrary absolute host path is ever mounted** — the assumed shared mount does not exist.
> 3. `range_inventory_ladder.py` — `Path("data") / file_name`; pathlib **drops the left operand** on an absolute right operand, so the ledger path escapes `data/` exactly as written.
> 4. **Net:** ledger written into the container's **ephemeral layer**, and the resumed bot **fresh-initializes from the wallet** — the insufficient-funds re-seed the hook exists to prevent — while preview reports success. The hook cannot distinguish "operator really mounts that path" from "path silently vanishes", and resolves the ambiguity by proceeding.
>
> Note §4's own table cites `state_path` at `range_inventory_ladder.py:1471` as the ledger derivation and marks it ✓ — the composition it validates is precisely the one that drops `data/`.
>
> Fix status: engine half **live** (batch phase 3, CONTRACT C1 — lexical rejection of absolute/drive/UNC/root/`..`/`.` under both `PurePosixPath` and `PureWindowsPath`, an explicit `allow_absolute_state_file_name: bool = False` opt-out for genuine shared-mount operators, plus a runtime resolve-and-containment assertion that raises at the use-site). API half in Run B: same accept/reject set in CFH planning, and **remove the absolute-skip success path**. Each half is independently fail-closed.
>
...
- **Type/falsy cluster (#1, #3, #4):** ~~compare ids as `str(...)` on both sides~~; replace `if not controller_id:` / `if not owner_id:` with `is None` (plus explicit empty-string handling); normalize `decisions` / `deployed_ids` keys to `str` so int and str ids can't split a controller into two buckets.
  ...
  > **[CORRECTION 2026-07-16 — hb_predeploy adversarial review + scope triage]** (correction 3) — **the struck `str(...)`-on-both-sides comparison is UNSOUND and must not be applied.** Both engines, independently: it **widens the API's accepted domain past the engine's contract**. Because the engine *rejects* a numeric `id:` (correction 2), coercing both sides makes the hook resolve `id: 123` as `copied` and start a container that then **crashes on engine-side config validation** — trading a clean, actionable 409 for a started-then-dead bot. Fix the **contract** instead (engine `min_length=1` + strip validator, **live**, batch phase 2; API rejects-and-aborts in Run B). The `is None` change and `str`-normalizing `decisions`/`deployed_ids` **keys** are unaffected — bucketing is not an identity-acceptance decision. Sources: `scope_triage.md` Batch 1 CDX-008 + "Doc corrections owed"; `REPORT_hb_predeploy.md` §1.2, §4.7.
...
- **#2:** in `_plan_extra_paths` (and/or the P1 model validator) reject ~~`""` and~~ `.`, and require each extra path to resolve to a **strict subpath** of source `data/` (reject `resolved == data_root` and directory-valued root).
  ...
  > **[CORRECTION 2026-07-16 — hb_predeploy adversarial review + scope triage]** (correction 4) — **`""` is already rejected 422 at the model layer** (`models/bot_orchestration.py:124-125`, `if not p: raise ValueError(f"Empty path in {label}")`, verified at `hummingbot-api` HEAD `b3aad36`), so it never reaches `_plan_extra_paths` and needs no fix there. The real trigger is **`"."` — and `"./"`, which both docs miss**. **This item is OUT OF SCOPE / accepted risk** (CDX-014 / CLA-005, Medium): not present in this operator's configs, and `resume_extra_paths` is an advanced field this operator does not set — this correction fixes the doc's stated trigger and is **not** an instruction to implement it now. If ever addressed, the predicate must be **"normalizes to the data root"** (any entry whose `PurePosixPath` normalizes to `.`/empty) plus a strict-descendant check — not a literal `("", ".")` denylist, which `"./"` and `"a/.."` walk straight through. Sources: `scope_triage.md` OUT OF SCOPE (CDX-014 / CLA-005) + "Doc corrections owed"; `REPORT_hb_predeploy.md` §1.2, §4.11.
