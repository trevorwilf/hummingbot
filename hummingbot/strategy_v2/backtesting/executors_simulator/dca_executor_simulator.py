...
class DCAExecutorSimulator(ExecutorSimulatorBase):
    ...
    def simulate(self, df: pd.DataFrame, config: DCAExecutorConfig, trade_cost: float) -> ExecutorSimulation:
        ...
        for i, dca_stage in enumerate(potential_dca_stages):
            # Each stage's fills start at ITS OWN entry timestamp — the loop variable
            # `entry_timestamp` leaked from the discovery loop above holds only the
            # LAST stage's entry and backdated every stage's volume/PnL to it.
            stage_entry_timestamp = dca_stage['entry_timestamp']
            df_filtered.loc[stage_entry_timestamp:, f'filled_amount_quote_{i}'] = dca_stage['amount']
            df_filtered.loc[stage_entry_timestamp:, f'net_pnl_quote_{i}'] = dca_stage['cumulative_returns'] * dca_stage['amount']
            df_filtered.loc[stage_entry_timestamp:, 'current_position_average_price'] = dca_stage['break_even_price']
