...
def mqtt_serialize(payload: Dict[str, Any]) -> str:
    """Serialize an MQTT payload exactly as the previous transport did (ujson + primitives)."""
...
class TopicSpecs:
    ...
    HEARTBEATS: str = '/hb'
    # MQTT multi-level wildcard ('#'); the previous transport used '*' and converted it internally.
...
class MQTTCommands:
    ...
    def _on_cmd_start(self, msg: StartCommandMessage.Request):
        ...
        timeout = 30
        try:
            # Fork: strategy state is read from TradingCore, and this fork's start
            # command takes a single `v2_conf` (not upstream's script/conf split).
            if self._hb_app.trading_core.strategy_name is None and msg.v2_conf is None:
    ...
    def _on_cmd_config(self, msg: ConfigCommandMessage.Request):
        ...
        try:
            ...
            # Fork: config is read from the public strategy_config_map property.
            if isinstance(self._hb_app.strategy_config_map, dict):  # pragma: no cover
                for key, value in self._hb_app.strategy_config_map.items():
                    if isinstance(value, ConfigVar):
                        strategy_config[key] = value.value
                    else:
                        strategy_config[key] = value
            elif isinstance(self._hb_app.strategy_config_map,
                            ClientConfigAdapter):
                strategy_config = self._hb_app.strategy_config_map.dict()
            response.config = {
    ...
    def _on_cmd_status(self, msg: StatusCommandMessage.Request):
        ...
        timeout = 30  # seconds
        try:
            # Fork: strategy state is read from TradingCore.
...
class MQTTGateway:
    ...
    def _wrap_response(self, response) -> Dict[str, Any]:
        # Reply envelope preserved byte-for-byte from the previous transport so
        # existing consumers keep working. The literal "agent" value below is
        # part of that wire contract and must not change.
        return {
            "header": {
                "reply_to": "",
...
class MQTTExternalEvents:
    ...
    def _on_message(self, topic: str, payload: Dict[str, Any]) -> None:
        ...
        # object with a `.data` attribute (the previous msg_type behaviour).
        try:
